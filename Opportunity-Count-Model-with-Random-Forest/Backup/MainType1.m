...
% put previous 2 info in a row. For opp=2, append info in opp=1; for opp=3,
...
for id = 1:length(skills)
    ...
    for id2 = 1: length(students)
        ...
        ppf = subsub(:,5:9);
        ppf1 = [nan*ones(1,5);subsub(1:end-1,5:9)];
        if sum(index)<2
        ppf2 = [nan*ones(1,6);subsub(1:end-2,5:10)];   
        else
            ppf2 = [nan*ones(2,6);subsub(1:end-2,5:10)]; 
        end
        end
        PPF=[PPF;ppf2,ppf1,ppf];
    end
end
data = [data(:,1:4),PPF];
%% Random Forest
...
% i_s = 10;
% 4,5-10,5-9,5-9
    indicator = [true, false, false,false,false,true,false,false, false,...
...
%     NF = 2;
    RF = TreeBagger(nTree,data_train,result_train,'method','regression','oobpred','on','oobvarimp','on',...
        'CategoricalPredictors', indicator );
    tmp =  oobError(RF);
        error=[error,tmp(end)];
