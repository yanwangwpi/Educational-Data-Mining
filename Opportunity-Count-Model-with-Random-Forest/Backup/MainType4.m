...
for id = 1:length(skills)
    ...
    for id2 = 1: length(students)
        ...
        ppf = subsub(:,5:9);
        ppf_seq=ppf;
        num_subsub = size(subsub,1);
        for id3 = 1:prev
            if num_subsub>id3
                ppf_seq = [[nan*ones(id3,5);ppf(1:end-id3,:)],ppf_seq];
            else
                ppf_seq = [nan*ones(num_subsub,5),ppf_seq];
            end
        end
        PPF = [PPF;ppf_seq];
    end
...
% i_s = 10;
% 4,5-10,5-9,5-9
    indicator = [true,[false,false,false,false,true]...
...
%     index_skill = data_test(:,1)==skills(id_skill);
    ...
    residual_skill =[result_test,result_predict,data(test,i_opp)];
%     end
