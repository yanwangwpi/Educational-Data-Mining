    ...
    for id2 = 1: length(students)
        ...
        for num = 1:prev 
            if sum(index)>num
                ...
                ppf1 = [nan*ones(sum(index),1)];
                ppf2=[];
            end
            tmpppf =[tmpppf,[ppf1;ppf2]];
        end
...
% find opp=1
index = data_origin(:,i_opp)<=6;% | data_origin(:,i_opp)==2 | data_origin(:,i_opp)==3;
...
for i = 1:N
    ...
    % 6ms_first_response	7hint_per 8first_action
    indicator = [true,false, false,false,false,false,false,false,false,false,true];
    err_tol=[];
