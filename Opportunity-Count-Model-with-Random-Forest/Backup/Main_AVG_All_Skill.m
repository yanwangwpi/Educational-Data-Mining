...
for i = 1:N
    ...
    % 6ms_first_response	7hint_per 8first_action
    indicator = [false, false,false,false,true];
    err_tol=[];
...
%     r2_tol = [r2_tol,rsquared(result_predict,result_test)];
    ...
    residual_all{i} = [residual_all{i};oppor.*ones(length(residual_skill),1),residual_skill];
%     length(residual)
end
end

first = residual_all{1};
rmse=[];
    for id_opp = 1:6
    rmse = [rmse,(mean(first(first(:,1)==id_opp,2).^2))^0.5];
    end
