...
for i = 1:N
    ...
    data_train = data(train,[1,4:end]);
    result_train = NPC(train);
    data_test = data(test,[1,4:end]);
...
% i_s = 10;
    ...
    indicator = [true, true, false, false,false,false,true,false];
...
%     index_skill = data_test(:,1)==skills(id_skill);
    ...
    residual_skill =[result_test,result_predict,data(test,1),i*ones(size(result_test))];
%     end
%     r2_tol = [r2_tol,rsquared(result_predict,result_test)];
%     rmse = [rmse,(mean(residual_skill.^2))^0.5];
    residual_all = [residual_all;residual_skill];
%     length(residual)
end
...
opp_rmse = [];
for i_opp = 1:15
    index = residual_all(:,3)==i_opp;
    opp_rmse = [opp_rmse,(sum((residual_all(index,1)-residual_all(index,2)).^2)/sum(index))^0.5];
end
