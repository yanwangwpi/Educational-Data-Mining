...
function [student_skill_pair,assess_best] =  func_skill_covariate(file_name)

%% The "Read in data" and "Preprocess data"  can be replaced if data is already generated
load (file_name);

% skill_id	student_id	percent correct within a skill	percent correct within a student	percent first item correct within a skill	percent first item correct within a student	mean log(ms) within a skill	mean log(ms) within a student	% ws within a skill	% ws within a student	practice opportunity	max practice opportunity for student_skill pair	correct	log(ms), -1 is for not mastery	ws (1 for ws, 0 for not, -1 not indeterminate)	cross validation group
...
metric_this_train=[];
metric_this_test=[];
index_set = [i_skillc, i_skillf, i_skillm, i_skillw];
for set_id  =1:4
