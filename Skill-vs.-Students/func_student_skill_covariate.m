...
function [student_skill_pair,assess_best] =  func_student_skill_covariate(file_name)
...
testc = data_new(:,i_skillvlc)==id;
trainc = ~testc;

data_new_1 = data_new(data_new(:,i_oc)==1,:);
testf = data_new_1(:,i_skillvlf)==id;
trainf = ~testf;
...
index_set2 = [i_skillc, i_skillf, i_skillm, i_skillw];
for set_id  =1:4
data_train = data_new(trainc,[index_set(set_id),index_set2(set_id),i_correct]);
data_test = data_new(testc,[index_set(set_id),index_set2(set_id),i_correct]);
itrain = data_train(:,1)==data_train(:,1) & data_train(:,2)==data_train(:,2);
itest = data_test(:,1)==data_test(:,1) & data_test(:,2)==data_test(:,2);

[metric_train, metric_test]=get_rsquare2(itrain,itest,data_train,data_test);
...
index_set2 = [i_skillc, i_skillf, i_skillm, i_skillw];
for set_id  =1:4
data_train = data_new_1(trainf,[index_set(set_id),index_set2(set_id),i_correct]);
data_test = data_new_1(testf,[index_set(set_id),index_set2(set_id),i_correct]);
itrain = data_train(:,1)==data_train(:,1) & data_train(:,2)==data_train(:,2);
itest = data_test(:,1)==data_test(:,1) & data_test(:,2)==data_test(:,2);
[metric_train, metric_test]=get_rsquare2(itrain,itest,data_train,data_test);
...
index_set2 = [i_skillc, i_skillf, i_skillm, i_skillw];
for set_id  =1:4
data_train = data_new_1(trainf,[index_set(set_id),index_set2(set_id),i_ms]);
data_test = data_new_1(testf,[index_set(set_id),index_set2(set_id),i_ms]);
itrain = data_train(:,1)==data_train(:,1) & data_train(:,2)==data_train(:,2) & data_train(:,3)~=-1;
itest = data_test(:,1)==data_test(:,1)& data_test(:,2)==data_test(:,2)& data_test(:,3)~=-1;

[metric_train, metric_test]=get_rsquare2_linear(itrain,itest,data_train,data_test);
metric_this_train =[metric_this_train,metric_train];
metric_this_test =[metric_this_test,metric_test];
...
index_set2 = [i_skillc, i_skillf, i_skillm, i_skillw];
for set_id  =1:4
data_train = data_new_1(trainf,[index_set(set_id),index_set2(set_id),i_ws]);
data_test = data_new_1(testf,[index_set(set_id),index_set2(set_id),i_ws]);
itrain = data_train(:,1)==data_train(:,1)& data_train(:,2)==data_train(:,2) & data_train(:,3)~=-1;
itest = data_test(:,1)==data_test(:,1)& data_test(:,2)==data_test(:,2)& data_test(:,3)~=-1;
[metric_train, metric_test]=get_rsquare2(itrain,itest,data_train,data_test);
...
student_skill_pair=[length(unique(data_new_1(:,i_student))),length(unique(data_new_1(:,i_skill))),size(data_new_1,1)];
