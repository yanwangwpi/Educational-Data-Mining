function [metric_train, metric_test]=get_rsquare2(itrain,itest,data_train,data_test)
% Two predictors
...
if sum(itrain)~=0
    [b,dev1,stats] = glmfit(data_train(itrain,1:2),data_train(itrain,3),'binomial','link','logit');
    pred1 = glmval(b, data_train(itrain,1:2), 'logit');
    ...
    [b0,dev0,stats] = glmfit(predictor,data_train(itrain,3),'binomial','link','logit');
    metric_train.r2mc  = 1-dev1/dev0;
    
    if sum(itest)~=0
        predictor = zeros(sum(itest),1);
        pred2 = glmval(b, data_test(itest,1:2), 'logit');
        [metric_test.r2, metric_test.rmse,metric_test.r2ef,metric_test.var,metric_test.auc] = rsquare2(data_test(itest,3),pred2);
        
        dev1 = loglike(b,data_test(itest,1:2),data_test(itest,3));
        dev0 = loglike(b0,predictor,data_test(itest,3));
