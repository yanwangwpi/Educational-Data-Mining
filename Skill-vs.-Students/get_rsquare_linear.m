function [metric_train, metric_test]=get_rsquare_linear(itrain,itest,data_train,data_test)
% One predictor
...
if sum(itrain)~=0
lm = LinearModel.fit(data_train(itrain,1),data_train(itrain,2));
pred1 = predict(lm, data_train(itrain,1));
[metric_train.r2, metric_train.rmse,metric_train.r2ef,metric_train.var, metric_train.auc] = rsquare2(data_train(itrain,2),pred1);
if sum(itest)~=0
pred2 = predict(lm, data_test(itest,1));
