% This file is used to predict half of students' practice with the other
% half. This file includes 5 cross validation

clc;
clear all;
% close all;

load correct_table_algebra;
load feature_table_algebra;

limit_stu = 30;
limit  = 3;
limit_all = limit_stu*limit;

% Correct_table(student_id,sequence_id*limit_stu*limit, correct*limit_stu*limit)
% Feature_table(student_id, hint_usage*limit_stu*limit, hint_toal*limit_stu*limit, attempt*limit_stu*limit)

i_seq = 2;
i_correct = limit_all+2;
i_hint = 2;
i_att = limit_all+2;
...
pred_partial = partial_credit(pred_correct,pred_hint, ones(size(pred_hint))*3, pred_att);
